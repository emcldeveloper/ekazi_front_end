...
const AllFeaturedCandidate = () => {
  ...
  console.log("HAPA KUBABAKE", jobSeekers);
  ...
  const filteredCandidates = jobSeekers.filter((candidate) => {
    ...
    const applicantKnowledges =
      applicant?.applicant_knowledges?.map((k) =>
        k.knowledge?.knowledge_name?.toLowerCase().trim(),
      ) || [];
    ...
    const hasMatchingEducation =
      !filters.education ||
      applicant.educations?.some(
        (edu) => edu.education_level_id?.toString() === filters.education,
      );

    const hasMatchingLevel =
      !filters.level ||
      applicant.positions?.some(
        (pos) => pos.level?.id?.toString() === filters.level,
      );
    ...
    return (
      ...
      hasMatchingLevel &&
      (!filters.experience ||
        applicant.experience?.toString() === filters.experience) &&
      (!filters.industry ||
        applicant.positions?.some(
          (pos) => pos.industry?.id?.toString() === filters.industry,
        )) &&
      (!filters.gender || applicant.gender_id?.toString() === filters.gender) &&
      (!filters.region ||
        applicant.address?.region_id?.toString() === filters.region) &&
      (!filters.country ||
        applicant.address?.region?.country_id?.toString() === filters.country)
    );
  });
